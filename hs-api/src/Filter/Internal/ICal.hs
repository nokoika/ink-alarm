...
import Prelude (Maybe (Just, Nothing), String, show, ($), (++), (==))
...
createICalEventsFromDefaultSchedules queryRoot@Q.QueryRoot {utcOffset, filters, language} defaultSchedules mode =
  let settingsUrl = settingsUrlFromQuery queryRoot
   in uniqueById
        [ I.ICalEvent
...
settingsUrlFromQuery Q.QueryRoot {Q.language, Q.rawQuery = Just rawQuery} =
  "https://ink-alarm.pages.dev/"
    ++ languagePath language
    ++ "?query="
    ++ Text.unpack rawQuery
settingsUrlFromQuery Q.QueryRoot {Q.language, Q.rawQuery = Nothing} =
  "https://ink-alarm.pages.dev/" ++ languagePath language
